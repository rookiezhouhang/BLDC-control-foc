...
float PID_velocity(float error)
{
	...
	now_us = SysTick->VAL;
	if(now_us<pid_vel_timestamp)Ts = (float)(pid_vel_timestamp - now_us)/12000000.0f;
	else
		Ts = (float)(0xFFFFFF - now_us + pid_vel_timestamp)/12000000.0f;
...
float PID_angle(float error)
{
	...
	now_us = SysTick->VAL;
	if(now_us<pid_ang_timestamp)Ts = (float)(pid_ang_timestamp - now_us)/12000000.0f;
	else
		Ts = (float)(0xFFFFFF - now_us + pid_ang_timestamp)/12000000.0f;
