...
float velocityOpenloop(float target_velocity)
{
	...
	now_us = SysTick->VAL; //¶Á¼Ä´æÆ÷µÄÖµ
	if(now_us<open_loop_timestamp)Ts = (float)(open_loop_timestamp - now_us)/36*1e-6;
	else
		Ts = (float)(0xFFFFFF - now_us + open_loop_timestamp)/36*1e-6;
